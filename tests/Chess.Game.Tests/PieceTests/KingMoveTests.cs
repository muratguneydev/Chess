	...
	private class KingMoveTestDataCollection
	{
		public static IEnumerable TestCases
		{
			get
			{
				...
				yield return new MoveUsingBoardTestData(board => new Move(board.a1, board.a2))
					.SetName("Move up if not blocked");
				yield return new MoveUsingBoardTestData(board => new Move(board.a2, board.a1))
					.SetName("Move down if not blocked");
				yield return new MoveUsingBoardTestData(board => new Move(board.a1, board.b1))
					.SetName("Move right if not blocked");
				yield return new MoveUsingBoardTestData(board => new Move(board.b1, board.a1))
