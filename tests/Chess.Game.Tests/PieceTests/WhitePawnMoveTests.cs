...
public class WhitePawnMoveTests
{
	...
	private class WhitePawnMoveTestDataCollection
	{
		public static IEnumerable TestCases
		{
			get
			{
				yield return new FromToUsingBoardTestData(board => new Move(board.a2, board.a4))
					.SetName("Move 2 squares as first move if not blocked");
				yield return new FromToUsingBoardTestData(board => new Move(board.a2, board.a3))
					.SetName("Move 1 square as first move if not blocked");
				yield return new FromToUsingBoardTestData(board => new Move(board.a3, board.a4))
					.SetName("Move 1 square after the first move a3-a4 if not blocked");
				yield return new FromToUsingBoardTestData(board => new Move(board.a4, board.a5))
